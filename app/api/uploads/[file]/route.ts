 ...
 *
 * A product photograph is the one exception: the menu is public on every tablet, so a photo a
 * product uses is served to anyone, and cached as public. Everything else (delivery notes, staff
 * photos) needs a Console session.
 */
export async function GET(_request: Request, { params }: { params: Promise<{ file: string }> }) {
  await fresh();
  const { file } = await params;
  const menuPhoto = catalogue.products().some((p) => p.imageKey === `/api/uploads/${file}` && p.status === 'active');
  if (!menuPhoto) {
  ...
  return new Response(new Uint8Array(stored.bytes), {
    headers: {
      ...
      'cache-control': `${menuPhoto ? 'public' : 'private'}, max-age=31536000, immutable`,
