 ...
 * the initial set on the category's colour. Decorative; the name beside it is the label.
...
export function ProductThumb({ name, imageKey, colour, size = 'sm' }: { name: string; imageKey: string | null; colour?: CategoryColourToken | null; size?: 'sm' | 'md' }) {
  ...
  return (
    <span
      aria-hidden="true"
      className={cx('relative flex shrink-0 items-center justify-center overflow-hidden rounded-md', src ? 'bg-thumb' : colour ? categoryEdgeClass(colour) : 'bg-thumb', size === 'md' ? 'size-control-lg' : 'size-control-md')}
    >
      {src ? (
        ...
        <span className={cx('text-label font-medium', colour ? 'text-seat-ink' : 'text-ink-subtle')}>{name.trim().slice(0, 1).toUpperCase()}</span>
      )}
    </span>
  );
}
