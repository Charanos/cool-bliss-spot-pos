...
export function ReceiptsTable({ rows, timezone, suppliers }: { rows: ReceiptRow[]; timezone: string; suppliers: { value: string; label: string }[] }) {
  const columns: Column<ReceiptRow>[] = [
    ...
    { key: 'value', header: 'Value at cost', width: '120px', align: 'right', sortValue: (r) => r.value, csv: (r) => formatDecimal(r.value), cell: (r) => <Money value={r.value} currency={false} size="num-md" decimals="whole" /> },
  ];
  return (
    <DataTable
